...
func (p ConcreteJobScriptProvider) NewScript(jobName string, scriptName string, options map[string]interface{}) Script {
	...
	return NewScript(p.fs, p.cmdRunner, jobName, path, stdoutLogPath, stderrLogPath, options)
}
