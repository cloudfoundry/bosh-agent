...
func (p ConcreteJobScriptProvider) NewScript(jobName string, scriptName string, env map[string]string) Script {
	...
	return NewScript(p.fs, p.cmdRunner, jobName, path, env, stdoutLogPath, stderrLogPath)
}
