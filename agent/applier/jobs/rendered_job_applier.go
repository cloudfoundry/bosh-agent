...
func (s *renderedJobApplier) downloadAndInstall(job models.Job, jobBundle boshbc.Bundle) error {
	file, err := s.blobstore.Get(boshcrypto.MustNewMultipleDigest(job.Source.Sha1), job.Source.SignedURL.Reveal(), job.Source.BlobstoreID, job.Source.BlobstoreHeaders)
	if err != nil {
