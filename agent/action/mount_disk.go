...
type diskMounter interface {
	...
	IsMountPoint(path string) (string, bool, error)
}
...
func (a MountDiskAction) Run(diskCid string) (interface{}, error) {
	...
	devicePath, isMountPoint, err := a.diskMounter.IsMountPoint(mountPoint)
	if err != nil {
	...
	if isMountPoint {
		if fmt.Sprint(realPath+"1") == devicePath {
			a.logger.Info(logTag, "device: %s is already mount to %s, skipping mounting", devicePath, mountPoint)
