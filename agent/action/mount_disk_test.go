		...
		Context("when settings can be loaded", func() {
			Context("when disk cid can be resolved to a device path from infrastructure settings", func() {
				...
				It("checks if store directory is already mounted", func() {
					...
					Expect(platform.FindDeviceMatchingMountPointPath).To(Equal("/fake-base-dir/store"))
				})

				Context("when store directory is not mounted", func() {
					BeforeEach(func() {
						platform.FindDeviceMatchingMountPointResult = false
					})
				...
				Context("when store directory is already mounted", func() {
					BeforeEach(func() {
						platform.FindDeviceMatchingMountPointResult = true
						platform.FindDeviceMatchingMountPointDevice = "fake-device-path"
				...
				Context("when store directory cannot be determined if it is mounted", func() {
					BeforeEach(func() {
						platform.FindDeviceMatchingMountPointErr = errors.New("fake-is-mount-point-err")
					})
