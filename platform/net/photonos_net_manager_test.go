...
`
	...
	Describe("GetConfiguredNetworkInterfaces", func() {
		Context("when there are network devices", func() {
			BeforeEach(func() {
				netmgrGetLinkInfo := "Name      	MacAddress       	Mode      	MTU       	State    \n"
				netmgrGetLinkInfo = netmgrGetLinkInfo + writeOutputLine("fake-eth0", "aa:bb")
				netmgrGetLinkInfo = netmgrGetLinkInfo + writeOutputLine("fake-eth1", "cc:dd")
			...
			It("returns networks that are configured by netmgr", func() {

				interfaces, err := netManager.GetConfiguredNetworkInterfaces()
				Expect(err).ToNot(HaveOccurred())

				Expect(interfaces).To(ConsistOf("fake-eth0", "fake-eth1"))
			})
		})
