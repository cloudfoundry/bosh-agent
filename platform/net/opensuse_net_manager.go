...
func (net OpensuseNetManager) SetupNetworking(networks boshsettings.Networks, errCh chan error) error {
	...
	dnsChanged := false
	dnsChanged, err = net.writeDNSConfiguration(dnsServers)
	if err != nil {
		return err
	}

	if interfacesChanged || dnsChanged {
...
// Global network config file - /etc/sysconfig/network/config
const opensuseNetworkConfigTemplate = `# Generated by bosh-agent
WICKED_DEBUG=""
WICKED_LOG_LEVEL=""
...
func (net OpensuseNetManager) writeDNSConfiguration(newServers []string) (bool, error) {
	changed := false
	dnsServers := []string{}
	buffer := bytes.NewBuffer([]byte{})
	t := template.Must(template.New("dns-config").Parse(opensuseNetworkConfigTemplate))
	...
	if net.fs.FileExists(dhclientConfigFile) {
		...
		allServers = append(newServers, allServers...)
