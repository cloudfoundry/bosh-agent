...
func (net UbuntuNetManager) writeNetworkInterfaces(
	...
	for _, dynamicAddressConfiguration := range dhcpConfigs {
		changed, err := net.writeDynamicInterfaceConfiguration(dynamicAddressConfiguration, dnsServers)
		if err != nil {
	...
	for _, staticAddressConfiguration := range staticConfigs {
		changed, err := net.writeStaticInterfaceConfiguration(staticAddressConfiguration, dnsServers)
		if err != nil {
...
func (net UbuntuNetManager) writeStaticInterfaceConfiguration(config StaticInterfaceConfiguration, dnsServers []string) (bool, error) {
...
func (net UbuntuNetManager) writeDynamicInterfaceConfiguration(config DHCPInterfaceConfiguration, dnsServers []string) (bool, error) {
	finalFile, err := net.fs.OpenFile(interfaceConfigurationFile(config.Name), os.O_CREATE|os.O_RDWR, 0666)
	if err != nil {
		return false, err
	}
