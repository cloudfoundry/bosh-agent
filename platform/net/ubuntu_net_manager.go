...
func (net UbuntuNetManager) writeNetworkInterfaces(
	...
	for _, dynamicAddressConfiguration := range dhcpConfigs {
		...
		changed, err := net.updateConfiguration(dynamicAddressConfiguration.Name, systemdDynamicNetworkTemplate, networkInterfaceConfiguration, opts)
		if err != nil {
	...
	for _, staticAddressConfiguration := range staticConfigs {
		changed, err := net.writeStaticInterfaceConfiguration(staticAddressConfiguration, true, dnsServers)
		if err != nil {
...
func (net UbuntuNetManager) writeStaticInterfaceConfiguration(config StaticInterfaceConfiguration, hasDnsNameServers bool, dnsServers []string) (bool, error) {
	finalFile, err := net.fs.OpenFile(interfaceConfigurationFile(config.Name), os.O_CREATE|os.O_RDWR, 0666)
...
const systemdDynamicNetworkTemplate = `# Generated by bosh-agent
[Match]
