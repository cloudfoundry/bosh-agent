...
func (net UbuntuNetManager) SetupNetworking(networks boshsettings.Networks, mbus string, errCh chan error) error {
	if networks.IsPreconfigured() {
		...
		err := net.updateDNS(dnsNetwork.DNS)
		if err != nil {
			return err
		}
...
// updateDNS takes a list of nameservers and attempts to discover
// whether the underlying OS is using the old resolveconf package
// (jammy jellyfish) or the newer systemd-resolved (noble numbat)
// and then set the DNS servers accordingly
func (net UbuntuNetManager) updateDNS(dnsServers []string) error {
	_, err := net.fs.Stat("/etc/resolvconf/resolv.conf.d/") // only exists for resolveconf
	if err != nil {
		return net.writeSystemdResolved(dnsServers)
	} else {
		return net.writeResolvConf(dnsServers)
	}
}

func (net UbuntuNetManager) writeSystemdResolved(dnsServers []string) error {
	// mkdir /etc/systemd/resolved.conf.d/
	// create /etc/systemd/resolved.conf.d/10-bosh.conf
	// contents (if your dns Servers are 1.1.1.1 and 8.8.8.8):
	// DNS=8.8.8.8 1.1.1.1
	if len(dnsServers) == 0 {
		// No servers to configure, so return
		// Fixme: why does writeResolvConf() do so much when there are no servers? Am I missing something?
		return nil
	}
	...
	err = net.fs.WriteFile(systemdResolvedPath, buffer.Bytes())
	if err != nil {
