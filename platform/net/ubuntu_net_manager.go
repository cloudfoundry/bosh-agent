...
func (net UbuntuNetManager) writeNetworkInterfaces(
	...
	staticConfigsForOneInterface := make(map[string]StaticInterfaceConfigurations)
	for _, staticAddressConfiguration := range staticConfigs {
		staticConfigsForOneInterface[staticAddressConfiguration.Name] = append(
			staticConfigsForOneInterface[staticAddressConfiguration.Name],
			staticAddressConfiguration,
		)
	}

	for interfaceName, staticAddressConfigurations := range staticConfigsForOneInterface {
		changed, err := net.writeStaticInterfaceConfiguration(staticAddressConfigurations, dnsServers, opts)
		if err != nil {
			return false, bosherr.WrapError(err, fmt.Sprintf("Updating network configuration for %s", interfaceName))
		}
...
func (net UbuntuNetManager) writeStaticInterfaceConfiguration(configs StaticInterfaceConfigurations, dnsServers []string, opts boshsys.ConvergeFileContentsOpts) (bool, error) {
	...
	// A deterministic order keeps the rendered file stable across agent runs:
	// IPv4 addresses first, then IPv6, ordered by address within each family.
	sort.SliceStable(configs, func(i, j int) bool {
		if configs[i].IsVersion6() != configs[j].IsVersion6() {
			return !configs[i].IsVersion6()
		}
		return configs[i].Address < configs[j].Address
	})
	...
	// Virtual Interfaces
	seenVirtualInterfaces := map[VirtualInterface]bool{}
	for _, config := range configs {
		for _, virtualInterface := range config.VirtualInterfaces {
			if seenVirtualInterfaces[virtualInterface] {
				continue
			}
			seenVirtualInterfaces[virtualInterface] = true
