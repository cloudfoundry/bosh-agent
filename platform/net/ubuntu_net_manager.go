...
{{ range .DHCPConfigs }}
auto {{ .Name }}
iface {{ .Name }} inet dhcp{{ $ifaceName := .Name }}{{ range .PostUpRoutes }}
post-up ip route add {{ .Destination }}/{{ .NetmaskSize }} via {{ .Gateway }} dev {{ $ifaceName }}{{ end }}
...
iface {{ .Name }} inet{{ .Version6 }} static
    ...
    post-up route -A inet6 add -net {{ .Destination }} netmask {{ .Netmask }} gw {{ .Gateway }}{{ end }}{{ else }}{{ $ifaceName := .Name }}{{ range .PostUpRoutes }}
    post-up ip route add {{ .Destination }}/{{ .NetmaskSize }} via {{ .Gateway }} dev {{ $ifaceName }}{{ end }}{{ end }}
{{ end }}{{ if .HasVersion6 }}
