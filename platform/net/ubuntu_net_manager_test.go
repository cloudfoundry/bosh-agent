...
dns-nameservers 8.8.8.8 9.9.9.9`
		...
		Context("networks is preconfigured", func() {
			...
			Context("when there are configured DNS servers", func() {
				...
				It("writes DNS to /etc/resolvconf/resolv.conf.d/head", func() {
					err := netManager.SetupNetworking(networks, "", nil)
					Expect(err).ToNot(HaveOccurred())

					resolvConfBase := fs.GetFileTestStat("/etc/resolvconf/resolv.conf.d/head")
...
`
				...
				Context("when writing to ../resolv.conf.d/head fails", func() {
					It("fails reporting the error", func() {
						fs.WriteFileError = errors.New("fake-write-file-error")

						err := netManager.SetupNetworking(networks, "", nil)
						Expect(err).To(HaveOccurred())
						Expect(err.Error()).To(ContainSubstring("Writing to /etc/resolvconf/resolv.conf.d/head"))
					})
				})
			})
