...
`
		...
		ItWritesNetworkIntefaces := func() {
			Context("when the Ubuntu version fails to be determined", func() {
				BeforeEach(func() {
					fs.RegisterReadFileError("/etc/lsb-release", errors.New("fake-read-file-error"))
				})

				It("fails with err: Invalid Ubuntu major version", func() {
					err := netManager.SetupManualNetworking(networks, nil)
					Expect(err).To(HaveOccurred())
					Expect(err.Error()).To(ContainSubstring("Invalid Ubuntu major version"))
				})
			})

			Context("when Ubuntu version is 14.04 (trusty)", func() {
				BeforeEach(func() {
					fs.WriteFileString("/etc/lsb-release", "DISTRIB_ID=Ubuntu\nDISTRIB_RELEASE=14.04")
				})
			...
			Context("when Ubuntu version is 10.04 (lucid) and earlier", func() {
				BeforeEach(func() {
				...
				It("writes DNS servers in /etc/network/interfaces", func() {
					...
					networkConfig := fs.GetFileTestStat("/etc/network/interfaces")
					Expect(networkConfig).ToNot(BeNil())
					Expect(networkConfig.StringContents()).To(Equal(
