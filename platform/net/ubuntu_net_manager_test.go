...
var _ = Describe("ubuntuNetManager", func() {
	...
	Describe("ComputeNetworkConfig", func() {
		...
		Context("when interface alias exists in network settings", func() {
			It("static interface configuration should be constructed by alias name", func() {
				networks := boshsettings.Networks{
					"default": factory.Network{
						IP:      "10.10.0.32",
						Netmask: "255.255.255.0",
...
`
			...
			err := fs.WriteFileString("/etc/network/interfaces", expectedNetworkConfigurationForStaticAndDhcp)
			Expect(err).NotTo(HaveOccurred())
			err = fs.WriteFileString("/etc/dhcp/dhclient.conf", initialDhcpConfig)
			Expect(err).NotTo(HaveOccurred())
...
`))
		...
		It("writes a single network file when two manual networks share an interface", func() {
			sharedMAC := "fake-shared-mac-address"
			staticIPv4Network := boshsettings.Network{
				Type:    "manual",
				IP:      "1.2.3.4",
				Default: []string{"gateway", "dns"},
				Netmask: "255.255.255.0",
			...
			staticIPv6Network := boshsettings.Network{
				Type:    "manual",
				IP:      "fd7a:eeee:e2e4:1::4",
				Netmask: "ffff:ffff:ffff:ffff:0000:0000:0000:0000",
				Gateway: "fd7a:eeee:e2e4:1::1",
				Mac:     sharedMAC,
			}

			stubInterfaces(map[string]boshsettings.Network{
				"ethshared": staticIPv4Network,
			})

			interfaceAddrsProvider.GetInterfaceAddresses = []boship.InterfaceAddress{
				boship.NewSimpleInterfaceAddress("ethshared", "1.2.3.4"),
				boship.NewSimpleInterfaceAddress("ethshared", "fd7a:eeee:e2e4:1::4"),
			}

			err := netManager.SetupNetworking(boshsettings.Networks{
				"ipv4-network": staticIPv4Network,
				"ipv6-network": staticIPv6Network,
			}, nil, nil)
			Expect(err).ToNot(HaveOccurred())

			matches, err := fs.Ls("/etc/systemd/network/")
			Expect(err).ToNot(HaveOccurred())
			Expect(matches).To(ConsistOf("/etc/systemd/network/10_ethshared.network"))

			networkConfig := fs.GetFileTestStat("/etc/systemd/network/10_ethshared.network")
			Expect(networkConfig).ToNot(BeNil())
			Expect(networkConfig.StringContents()).To(Equal(`# Generated by bosh-agent
[Match]
Name=ethshared
...
`))
		...
		It("orders addresses deterministically and merges routes when manual networks share an interface", func() {
			sharedMAC := "fake-shared-mac-address"
			firstStaticNetwork := boshsettings.Network{
				Type:    "manual",
				IP:      "10.10.0.4",
				Default: []string{"gateway", "dns"},
				Netmask: "255.255.255.0",
			...
			stubInterfaces(map[string]boshsettings.Network{
				"ethshared": firstStaticNetwork,
			})

			interfaceAddrsProvider.GetInterfaceAddresses = []boship.InterfaceAddress{
				boship.NewSimpleInterfaceAddress("ethshared", "10.10.0.4"),
				boship.NewSimpleInterfaceAddress("ethshared", "10.20.0.4"),
			}

			err := netManager.SetupNetworking(boshsettings.Networks{
				"first-network":  firstStaticNetwork,
				"second-network": secondStaticNetwork,
			}, nil, nil)
			Expect(err).ToNot(HaveOccurred())
	...
	Describe("GetConfiguredNetworkInterfaces", func() {
		Context("when there are network devices", func() {
			...
			It("returns networks that are defined in /etc/network/interfaces", func() {
				err := fs.WriteFileString("/etc/systemd/network/10_fake-eth0.network", ``)
				Expect(err).NotTo(HaveOccurred())
				err = fs.WriteFileString("/etc/systemd/network/10_fake-eth2.network", ``)
				Expect(err).NotTo(HaveOccurred())
