	...
	Describe("SetupNetworking", func() {
		BeforeEach(func() {
			...
			interfaceAddrsProvider.GetInterfaceAddresses = []boship.InterfaceAddress{
				...
				boship.NewSimpleInterfaceAddress("ethstatic4", "5.6.7.8"),
			}
		})
		...
		It("writes /etc/network/interfaces with static inet6 configuration when manual network is used", func() {
			...
			Expect(matches).To(ConsistOf(
				"/etc/systemd/network/09_ethstatic1.network",
				"/etc/systemd/network/09_ethstatic2.network",
				"/etc/systemd/network/09_ethstatic3.network",
			))
			networkConfig := fs.GetFileTestStat("/etc/systemd/network/09_ethstatic1.network")
...
`))
			networkConfig = fs.GetFileTestStat("/etc/systemd/network/09_ethstatic2.network")
			Expect(networkConfig).ToNot(BeNil())
			Expect(networkConfig.StringContents()).To(Equal(`# Generated by bosh-agent
[Match]
Name=ethstatic2
...
`))
			networkConfig = fs.GetFileTestStat("/etc/systemd/network/09_ethstatic3.network")
			Expect(networkConfig).ToNot(BeNil())
			Expect(networkConfig.StringContents()).To(Equal(`# Generated by bosh-agent
[Match]
Name=ethstatic3
...
`))
		...
		It("sets IPv4 and IPv6 gateways when multiple networks are marked as default gateway", func() {
			static1Net := boshsettings.Network{
				Type:    "manual",
				IP:      "2601:646:100:e8e8::103",
				Netmask: "ffff:ffff:ffff:ffff:0000:0000:0000:0000",
			...
			static4Net := boshsettings.Network{
				Type:    "manual",
				IP:      "5.6.7.8",
				Netmask: "255.255.255.0",
				Gateway: "9.10.11.12",
