...
func (p linux) SetTimeWithNtpServers(servers []string) (err error) {
	_, _, _, err = p.cmdRunner.RunCommand("sed", "-i", "/^server/d", "/etc/ntp.conf")
	if err != nil {
		return bosherr.WrapError(err, "Failed to update /etc/ntp.conf")
	}
	...
	if content != "" {
		_, _, _, err = p.cmdRunner.RunCommand("sh", "-c", fmt.Sprintf("echo -e '%s' >> /etc/ntp.conf", content))
		if err != nil {
			return bosherr.WrapError(err, "Failed to add new servers into /etc/ntp.conf")
		}
	}

	_, _, _, err = p.cmdRunner.RunCommand("service", "ntp", "restart")
	if err != nil {
		return bosherr.WrapError(err, "Failed to retart ntp service")
	}
