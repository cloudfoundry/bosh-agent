...
func (p linux) SetupNetworking(networks boshsettings.Networks) (err error) {
	...
	return p.writeResolvConf(networks)
}

// for test
func (p linux) writeResolvConf(networks boshsettings.Networks) error {

	buffer := bytes.NewBuffer([]byte{})

	const sl_ubuntuResolvConfTemplate = `# Generated by bosh-agent
{{ range .DNSServers }}nameserver {{ . }}
{{ end }}`

	t := template.Must(template.New("resolv-conf").Parse(sl_ubuntuResolvConfTemplate))
...
func (p linux) MountPersistentDisk(diskSetting boshsettings.DiskSettings, mountPoint string) error {
	...
	if !p.options.UsePreformattedPersistentDisk {
		...
		if strings.Contains(realPath, "/dev/mapper/") {
			partitionPath = realPath + "-part1" // softlayer multipath support
		}
...
func (p linux) UnmountPersistentDisk(diskSettings boshsettings.DiskSettings) (bool, error) {
	...
	if !p.options.UsePreformattedPersistentDisk {
		if strings.Contains(realPath, "/dev/mapper/") {
			realPath = realPath + "-part1" // softlayer multipath support
		} else {
			realPath += "1"
		}
	}
