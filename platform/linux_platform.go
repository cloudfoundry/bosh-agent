...
func (p linux) SetupLogrotate(groupName, basePath, size string) (err error) {
	...
	logrotateConfigPath := path.Join("/etc/logrotate.d", groupName)
	err = p.fs.WriteFile(logrotateConfigPath, buffer.Bytes())
	if err != nil {
		err = bosherr.WrapError(err, "Writing to /etc/logrotate.d")
		return
	}

	p.forceRotateLogFiles(logrotateConfigPath)
...
func (p linux) forceRotateLogFiles(configFilePath string) {
	_, _, _, err := p.cmdRunner.RunCommand("logrotate", "--force", configFilePath)
	if err == nil {
		p.logger.Info(logTag, "Successfully rotated job's log files")
	} else {
		p.logger.Error(logTag, "Could not rotate job's log files", err)
	}
}
