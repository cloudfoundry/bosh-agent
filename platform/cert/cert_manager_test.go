...
var _ = Describe("Certificate Management", func() {
	...
	Describe("cert.Manager implementations", func() {
		...
		Context("PhotonOS", func() {
			BeforeEach(func() {
				fakeFs = fakesys.NewFakeFileSystem()
				fakeCmdRunner = fakesys.NewFakeCmdRunner()
				fakeCmdRunner.AddCmdResult("/usr/bin/rehash_ca_certificates.sh", fakesys.FakeCmdResult{
					Stdout:     "",
					Stderr:     "",
					ExitStatus: 0,
					Sticky:     true,
				})
				certManager = cert.NewPhotonOSCertManager(fakeFs, fakeCmdRunner, 0, log)
			})

			SharedLinuxCertManagerExamples("/etc/ssl/certs", "/usr/bin/rehash_ca_certificates.sh")
