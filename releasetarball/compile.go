...
func NewCompiler(dirProvider directories.Provider) (boshcomp.Compiler, error) {
	...
	bd := blobstore_delegator.NewBlobstoreDelegator(httpblobprovider.NewHTTPBlobImpl(filesystem, http.DefaultClient), boshagentblobstore.NewCascadingBlobstore(db, nil, logger))
	ts := clock.NewClock()
