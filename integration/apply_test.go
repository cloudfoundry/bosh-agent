...
var _ = Describe("apply", func() {
	...
	It("should send agent apply and create appropriate /var/vcap/data directories for a job", func() {
		...
		// gating on one path existing and then re-reading to assert perms is a TOCTOU where the gate can
		// pass in the window before the agent has set ownership/mode on all of them.
