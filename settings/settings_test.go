...
var _ = Describe("Settings", func() {
  ...
  "bosh": {
	...
	Describe("#GetBlobstore", func() {
		Context("Env.Bosh.Blobstores is populated", func() {
			It("should return Env.Bosh.Blobstores[0]", func() {
				settings = Settings{
					Env: Env{
						Bosh: BoshEnv{
							Blobstores: []Blobstore{
								Blobstore{
									Type: "blobstore-provider",
								},
							},
						},
					},
					Blobstore: Blobstore{
						Type: "toplevel-blobstore",
					},
				}

				Expect(settings.GetBlobstore().Type).To(Equal("blobstore-provider"))
			})
		})

		Context("Env.Bosh.Blobstores is nil", func() {
			It("should return Env.Bosh.Blobstores[0]", func() {
				settings = Settings{
					Env: Env{
						Bosh: BoshEnv{
				...
				Expect(settings.GetBlobstore().Type).To(Equal("toplevel-blobstore"))
			})
		})

		Context("Env.Bosh.Blobstores is zero length", func() {
			It("should return Env.Bosh.Blobstores[0]", func() {
				settings = Settings{
					Env: Env{
						Bosh: BoshEnv{
