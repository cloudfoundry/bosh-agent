...
func init() {
	Describe("settingsService", func() {
		...
		Describe("GetPersistentDiskHints", func() {
			...
			Context("disk hints file does not exist on disk", func() {
				It("returns empty map of disk hints", func() {
					...
					Expect(fs.ReadFileWithOptsCallCount).To(Equal(0))
				})
			})
		...
		Describe("GetPersistentDiskHint", func() {
			var (
				fetchedSettings Settings
				fetcherFuncErr  error
				service         Service
			...
			Context("disk hints file does not exist on disk", func() {
				It("returns empty map of disk hints", func() {
					...
					diskHint, found, error := service.GetPersistentDiskHint("smurf")
					Expect(fs.ReadFileWithOptsCallCount).To(Equal(0))
					Expect(diskHint).To(Equal(DiskSettings{}))
			...
			Context("disk hints file exists", func() {
				...
				It("loads persistent hints from disk", func() {
					service.GetPersistentDiskHint("smurf")
					Expect(fs.ReadFileWithOptsCallCount).To(Equal(1))
				})
				...
				Context("has invalid hints saved on disk", func() {
					...
					It("returns error", func() {
						_, _, err := service.GetPersistentDiskHint("smurf")
						Expect(err).To(HaveOccurred())
						Expect(err.Error()).To(ContainSubstring("Unmarshalling persistent disk hints from file"))
					})
				})
				...
				Context("has valid hints saved on disk", func() {
					...
					Context("requested disk hint does not exist in file", func() {
						It("returns negative find result with an empty hint and no error", func() {
							diskHints, found, err := service.GetPersistentDiskHint("smurf")
							Expect(err).ToNot(HaveOccurred())
							Expect(found).To(BeFalse())
							Expect(diskHints).To(Equal(DiskSettings{}))
						})
					})

					Context("requested disk hint exists in file", func() {
						It("returns positive find result with the disk hint and no error", func() {
							diskHints, found, err := service.GetPersistentDiskHint("2")
							Expect(err).ToNot(HaveOccurred())
							Expect(found).To(BeTrue())
							Expect(diskHints).To(Equal(existingHintsOnDisk["2"]))
						})
					})
				})
			})
		...
		Describe("RemovePersistentDiskHint", func() {
			...
			Context("persistent disk hints file does not exist", func() {
				It("returns error", func() {
					...
					Expect(err).To(HaveOccurred())
					Expect(err.Error()).To(ContainSubstring("Cannot delete disk hint from file that does not exist"))
				})
			})
